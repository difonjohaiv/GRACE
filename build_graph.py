...
def count_pmi(windows_len, word_pair_count, word_window_freq, threshold):
    word_pmi_lst = list()
    for word_pair, W_i_j in tqdm(word_pair_count.items(),
                                 desc="Calculate pmi between words"):
...
def get_pmi_edge(content_lst, window_size=20, threshold=0.):
    ...
    pmi_edge_lst = count_pmi(windows_len, word_pair_count, word_window_freq,
                             threshold)
...
class BuildGraph:
    ...
    def get_pmi_edge(self):
        pmi_edge_lst, self.pmi_time = get_pmi_edge(self.content,
                                                   window_size=20,
                                                   threshold=0.0)
