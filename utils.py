...
class CudaUse(object):
    ...
    def gpu_mem_get_all(self):
        ...
        return list(
            map(self.gpu_mem_get, range(self.pynvml.nvmlDeviceGetCount())))
...
def print_table(dst):
    ...
    table = PrettyTable(field_names=table_title,
                        header_style="title",
                        header=True,
                        border=True,
                        hrules=1,
...
def preprocess_adj(adj, is_sparse=False):
    ...
    # sp.eye(adj.shape[0])添加自环,对邻接矩阵进行归一化处理
    adj_normalized = normalize_adj(adj + sp.eye(adj.shape[0]))
    if is_sparse:  # 将scipy稀疏矩阵转换为PyTorch稀疏张量
